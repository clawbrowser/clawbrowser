...
Environment overrides:
  ...
  HOME, XDG_CONFIG_HOME, XDG_CACHE_HOME, XDG_DATA_HOME
                             Set these to writable paths in restricted containers on persistent storage; the normal release archive is about 200 MB compressed and does not require a 2 GB preflight. Confirm at least 2 GB free only before fetching or unpacking portable runtime bundles, which are about 500 MB compressed. Do not use /tmp as the durable browser install location.
