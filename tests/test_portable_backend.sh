...
test_source_contract_for_browser_loader_binding() {
  ...
  grep -Fq -- 'portable_elf_interpreter_patchable' "${SOURCE_FILE}" || fail "portable browser helper ELF interpreter detection missing"
  ...
  grep -Fq -- 'portable_dri_driver_path' "${SOURCE_FILE}" || fail "portable Mesa DRI path helper missing"
  grep -Fq -- 'LIBGL_DRIVERS_PATH="${portable_libgl_drivers_path}"' "${SOURCE_FILE}" || fail "portable browser should use bundled Mesa DRI drivers"
  grep -Fq -- 'LIBGL_ALWAYS_SOFTWARE="${LIBGL_ALWAYS_SOFTWARE:-1}"' "${SOURCE_FILE}" || fail "portable browser should default to software GL in restricted containers"
  grep -Fq -- 'MESA_LOADER_DRIVER_OVERRIDE="${MESA_LOADER_DRIVER_OVERRIDE:-swrast}"' "${SOURCE_FILE}" || fail "portable browser should default to swrast Mesa driver"
  ...
  grep -Fq -- 'kill_process_tree "${saved_pid}" "${saved_xvfb_pid}"' "${SOURCE_FILE}" || fail "portable stop should clean up browser/Xvfb process trees"
  ...
  grep -Fq -- 'cleanup_portable_display_artifacts "${saved_display}"' "${SOURCE_FILE}" || fail "portable stop should clean stale X display artifacts"
